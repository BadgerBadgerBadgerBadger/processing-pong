package me.scionofbytes.processing.pong;
...
public class Main extends PApplet {
    ...
    private PFont titleF;
    private PFont menuItemF;

    // Creates the array that will be passed to the Menu class for forming individual MenuText objects.
    private String[] menuItemsNames = {"START", "INSTRUCTIONS", "CREDITS"};
    private Menu menu;
    ...
    private Ball pongScreenBall;
    private Ball pongCreditsBall;

    private Score tpScore;
    ...
    public void setup() {
        ...
        titleF = createFont("Arial", 30);
        menuItemF = createFont("Arial", 30);
        menu = new Menu(
            "PONG",
            menuItemsNames,
            titleF, menuItemF,
    ...
    class Ball {
        ...
        Ball(float radius, int bColor, float vBase) {
            this.radius = radius;
            this.bColor = bColor;
            this.vBase = vBase;
            ...
            if (p.getOrient() == 0) {
                if (bY >= (p.getY() - p.getHeight() / 2) && bY <= (p.getY() + p.getHeight() / 2)) {
                    if ((bX - radius) <= p.getWidth()) {
                        float varier = map(p.getY() - bY, 0, p.getHeight() / 2, 0.1f, 1);
    ...
    class Field {
        ...
        Field(int fEdge, int fFloor, int fNet) {
            this.fEdge = fEdge;
            this.fFloor = fFloor;
            this.fNet = fNet;
        }
    ...
    class Menu {
        ...
        float iniY = height / 2.5f;

        MenuText[] menuItems;

        Menu(String titleT, String[] menuItemsNames, PFont titleF, PFont menuItemF, int titleC, int menuItemC, int menuBackC, int itemHoverC, int backHoverC) {
            this.titleT = titleT;
            this.titleF = titleF;
            this.titleC = titleC;
        ...
        void render() {
            ...
            textSize(92);
            fill(titleC);
            text(titleT, width / 2 - (textWidth(titleT) / 2), height / 3.8f);
    ...
    /*
        MenuText holds the attributes and methods relating to each single item on the menu. Thus each item is treated as
        a separate object. Each MenuText object comprises mainly of a foreground text and a background object.
     */
    class MenuText {
        ...
        MenuText(String menuItem, PFont menuFont, float itemX, float itemY, float itemSize, int itemColor, int backColor, int pressedColor, int pressedBack) {
        ...
        void render() {
            ...
            fill(presentBack);
            rectMode(CENTER);
            rect(itemX, itemY, tWidth * 1.3f, itemSize * 1.4f, 50);

            fill(presentItem);
            text(menuItem, itemX - tWidth / 2, itemY + itemSize * 0.3f);
        }
    ...
    class Paddle {
        ...
        Paddle(float pWidth, float pHeight, int pColor, int pOrient) {
            this.pWidth = pWidth;
    ...
    class Score {
        ...
        Score(int maxScore, PFont pFont, int pSize, int pText, int pBack, int pStroke, float p1X, float p1Y, float p2X, float p2Y) {
            p1 = 0;
            p2 = 0;
        ...
        void render() {
            ...
            rect(p1X, p1Y, textWidth(str(p1)) * 1.3f, pSize);
            rect(p2X, p2Y, textWidth(str(p2)) * 1.3f, pSize);

            fill(pText);
            text(str(p1), p1X - (textWidth(str(p1)) / 2), p1Y + pSize * 0.3f);
            text(str(p2), p2X - (textWidth(str(p2)) / 2), p2Y + pSize * 0.3f);
        }
    ...
    public static void main(String[] args) {
        PApplet.main("me.scionofbytes.processing.pong.Main");
    }
}
